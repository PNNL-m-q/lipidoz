...
Feedback
----------------------
To report a bug or propose a new feature, please open an issue `here <https://github.com/PNNL-m-q/lipidoz/issues>`_. 
...
#. Go to the created ``lipidoz`` folder with :code:`cd lipidoz`
...
#. Push your changes to your ``lipidoz`` fork by running :code:`git push origin <your_github_username>_<type>_<descriptive_branch_name>`
...
Versioning Scheme
----------------------------
The version of ``lipidoz`` is stored in the ``__version__`` variable in ``lipidoz/__init__.py``.
...
.. note::
    ...
    to the current ``__version__`` variable in ``lipidoz/__init__.py``, then increment the number at the end with each 
...
Docstring Format
****************************
Detailed docstrings must be included in all functions/classes (both internal and external) in ``lipidoz``. Docstring 
...
above were implemented in the ``lipidoz/isotope_scoring.py`` module, then the following entry should be added to the 
appropriate section in the ``docs/source/isotope_scoring.rst`` documentation source file:
...
.. code-block::
    ...
    .. autofunction :: lipidoz.isotope_scoring.sum_pairwise
